...
export class SyncService {
  ...
  async update(data: UpdateDto & { user: User }) {
    const { user, categories: localCategories, transactions: localTransactions } = data
    ...
    await Promise.all(
      localTransactions
        .filter(transaction => {
          const savedTransaction = transactions.find(({ id }) => id === transaction.id)
          return !savedTransaction || moment(savedTransaction.updatedAt).isBefore(moment(transaction.updatedAt))
        })
        .map(transaction => {
